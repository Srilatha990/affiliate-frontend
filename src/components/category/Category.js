          ...
          <h2 className="font-semibold font-serif text-lg m-0 text-heading flex align-center">
            <Link href="/" className="mr-10">
              {/* <Image
                ...
                alt="logo"
              /> */}
              <h2 className="text-white">AffiliateInsight</h2>
            </Link>
          </h2>
          <button
            ...
            <div className="relative grid gap-1 p-6">
              {pages.map((item) => (
                <a
                  key={item.title}
                  href={item.href}
                  className="p-2 flex font-serif items-center rounded-md hover:bg-gray-50 w-full hover:text-orange-500"
                >
                  <item.icon
                    className="flex-shrink-0 h-4 w-4"
                    aria-hidden="true"
                  />
                  <p className="inline-flex items-center justify-between ml-2 text-sm font-medium w-full group-hover:text-[#0b5052] transition-colors">
                    {item.title}
                  </p>
                </a>
              ))}
